...
(function () {
  ...
  function totalGain(gameName, days, cfg) {
    ...
    for (let d = 0; d < days; d++) {
      let seed = d * 31;
      for (let i = 0; i < gameName.length; i++) seed += gameName.charCodeAt(i) * (i + 1);
  ...
  function updateCounts() {
    ...
    if (badge) {
      ...
      badge.textContent = '';
      if (icon) { badge.appendChild(icon); badge.append(' '); }
  ...
  function initOnlineCounter() {
    ...
    const hourlyCurve = [
      0.45, 0.35, 0.28, 0.22, 0.20, 0.22, 0.30, 0.48,
      0.62, 0.72, 0.78, 0.82, 0.85, 0.83, 0.80, 0.82,
      0.88, 0.93, 1.00, 1.00, 0.97, 0.90, 0.75, 0.58
    ];
    const dayCurve = [1.15, 0.90, 0.88, 0.92, 0.95, 1.05, 1.18];
    const BASE = 2800;
    ...
    setInterval(function () {
      ...
      const spike  = Math.random() < 0.03 ? Math.round((Math.random() - 0.3) * 60) : 0;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  (function () {
    ...
    var FIRST_SYL = [
      'Ja','Ma','Ro','Li','Mi','Sa','Da','Je','Ch','As','Ka','Ni','Ke','Am',
      'Br','Ti','An','De','Ca','So','Ty','Al','La','Ra','Mo','Le','Sha','Ta',
      'Dy','Jo','Pa','Na','Re','Ha','Za','Ky','Lu','Ar','El','Is','Ju','To',
      'Wa','Be','Gi','Vi','Co','Fe','Ri','Se','Te','We','No','Em','Ev','Ad'
    ];
    var FIRST_END = [
      'mes','ria','bert','nda','ke','rah','vid','ica','ris','ley','vin','cole',
      'son','nda','ian','ny','dre','tal','los','fia','ler','ney','ren','quel',
      'ses','ron','lly','sha','lan','na','niel','yla','rissa','drew','mone',
      'cus','leigh','gel','ion','don','vin','cey','lani','mond','trick','ette'
    ];
    var GAMES = [
      'Vegas Sweeps', 'Fire Kirin', 'Game Vault', 'Game Room777',
      'Orion Stars', 'Black Jack', 'Ultra Panda', 'Milky Ways',
      'V Power', 'Magic City', 'Blue Dragon'
    ];
    ...
    function generateName() {
      ...
      var lastInitial = String.fromCharCode(65 + Math.floor(seededRand() * 26)) + '.';
    ...
    function showToast() {
      ...
      toast.innerHTML =
        '<div class="winner-avatar">' + person.initial + '</div>' +
        '<div class="winner-info">' +
          '<div class="winner-name">' + person.full + '</div>' +
          '<div class="winner-detail">' + action + ' <strong>' + amount + '</strong> on ' + game + '</div>' +
        '</div>' +
